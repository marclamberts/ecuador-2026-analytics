...
Nodes = average pass-touch position per player (season-wide), sized by pass
volume. Edges = combined completed-pass count between two players in that
half. Only players with enough minutes are shown, to keep it to the core
squad rather than every substitute cameo.

Usage: python3 pass_network.py "Independiente del Valle" [out.png]
"""
import glob
import json
import re
...
def make_plot(team_name, files, cid, out_path):
    ...
    fig.text(0.03, 0.930, "Season Pass Network · First Half vs Second Half", fontsize=14.5,
    ...
    for ax, half in zip(axes, (1, 2)):
        ...
        for (a, b), cnt in edges[half].items():
            ...
            alpha = 0.25 + (cnt / max_edge) * 0.55
            pitch.lines(x0, y0, x1, y1, ax=ax, color=EDGE_COLOR, lw=lw, alpha=alpha,
        ...
        for pid, (x, y) in avg_pos.items():
            ...
            pitch.scatter(x, y, ax=ax, s=size, color=C_AMBER, edgecolors=BG,
...
if __name__ == "__main__":
    ...
    out = sys.argv[2] if len(sys.argv) > 2 else f"/tmp/pass_network_{team_name.replace(' ', '_')}.png"
