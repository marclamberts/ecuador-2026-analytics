"""
xG shot map builder for the Ecuador 2026 Opta F24-style event dataset.
Uses the pre-computed xG/PSxG values from Danger/*_danger_models.csv.

Usage: python3 shotmap.py "E. Mero"
"""
import csv
import glob
import json
...
def make_plot(player_name, shots, matches_played, out_path):
    ...
    total_psxg = sum(s["psxg"] for s in shots)
    xg_per_shot = total_xg / n_shots if n_shots else 0
    team = shots[0]["team"] if shots else "Unknown"

    by_bp = collections.defaultdict(lambda: [0, 0.0])
    for s in shots:
        by_bp[s["body_part"]][0] += 1
        by_bp[s["body_part"]][1] += s["xg"]
    ...
    for s in sorted(shots, key=lambda s: order.get(s["outcome"], 0)):
        ...
        pitch.scatter(s["x"], s["y"], ax=pitch_ax, s=size_for(s["xg"]),
    ...
    fig.text(0.02, 0.932, "Shot Map (xG) · excl. penalties", fontsize=13, color="#c7ccd4")
    ...
    for x, label, val in zip(plain_xs, ("xG", "xG/Shot", "PSxG"),
                              (f"{total_xg:.2f}", f"{xg_per_shot:.2f}", f"{total_psxg:.2f}")):
        fig.text(x, STAT_LABEL_Y, label, fontsize=11, ha="center", color="#d7dbe0")
        fig.text(x, STAT_VAL_Y, val, fontsize=18, fontweight="bold", ha="center", va="center", color="white")

    fig.text(PITCH_LEFT, 0.02,
             "xG / PSxG from the Danger model outputs (Danger/*_danger_models.csv).",
    ...
    y0 = RTOP
    section_header(y0, "SHOTS (xG) BY BODY PART")
    bp_items = [
    ...
    for i, (label, count, xg) in enumerate(bp_items):
        col = xs2[i % 2]
        row = i // 2
        yy = y0 - 0.045 - row * 0.062
        fig.text(col, yy, label, fontsize=9.5, color="#8b93a1")
        fig.text(col, yy - 0.026, f"{count}  ({xg:.2f})", fontsize=15, fontweight="bold", color="#ffffff")
    ...
    print(f"Shots={n_shots} Goals={n_goals} OnTarget={n_on} Blocked={n_blocked} OffTarget={n_off} "
          f"xG={total_xg:.2f} xG/shot={xg_per_shot:.3f} PSxG={total_psxg:.2f}")


if __name__ == "__main__":
    player = sys.argv[1] if len(sys.argv) > 1 else "E. Mero"
    out = sys.argv[2] if len(sys.argv) > 2 else f"/tmp/shotmap_{player.replace(' ', '_')}.png"
