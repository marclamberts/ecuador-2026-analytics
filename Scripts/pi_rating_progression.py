"""
Pi-rating progression across the season, ELO-style display (base 1500).
Top N teams by final rating are highlighted; the rest sit faint in grey.

Usage: python3 pi_rating_progression.py [out.png] [top_n]
"""
import sys
...
def make_plot(d, out_path, top_n=6):
    ...
    ax.axhline(pil.ELO_BASE, color=TEXT_SUB, linewidth=1.0, linestyle=(0, (2, 3)),
              alpha=0.5, zorder=1)

    for t in other_teams:
        hist = history[t]
        xs = [dt.datetime.strptime(h["date"], "%Y-%m-%d") for h in hist]
        ys = [pil.to_elo(h["combined"]) for h in hist]
    ...
    for i, t in enumerate(top_teams):
        ...
        label_specs.append({"x": xs[-1], "y": ys[-1], "color": color,
                            "text": f"{pil.clean_name(t)}  {ys[-1]:.0f}"})

    # declutter end-of-line labels: enforce a minimum vertical gap,
    # working from the highest rating down
    ...
    ax.set_ylabel("PI Rating (ELO scale)", fontsize=11.5, color=TEXT_MAIN, fontweight="bold", labelpad=10)
    ...
    fig.text(0.06, 0.965, "Ecuador 2026  ·  PI Rating Progression  (ELO Style)",
             fontsize=20, fontweight="bold", color="white")
    fig.text(0.06, 0.928, f"Rating updates after every match  ·  Base {pil.ELO_BASE} at season start  ·  "
             f"λ={pil.LAMBDA}, γ={pil.GAMMA}", fontsize=11, color=TEXT_SUB)
    fig.text(0.06, 0.905, f"Top {top_n} highlighted  ·  Others in grey", fontsize=9, color="#6b7684")
    ...
    for t in top_teams:
        print(f"{pil.clean_name(t):30s} {pil.to_elo(final_combined[t]):.1f}")


if __name__ == "__main__":
    out = sys.argv[1] if len(sys.argv) > 1 else "/tmp/pi_rating_progression.png"
